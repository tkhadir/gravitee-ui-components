...
export class GvPolicyStudioMenu extends LitElement {
  ...
  static get styles () {
    return [
      ...
      css`
        ...
        .entry:focus-within .actions gv-button {
          opacity: 1;
          visibility: visible;
          display: block;
        }

        .entry .actions gv-switch {
          opacity: 0.8;
        }

        .entry:focus-within .actions gv-switch {
          opacity: 1;
        }
  ...
  _renderFlowActions (content) {
    const enabled = content.disabled !== true;
  ...
  _findFlowCollection (flowId) {
    ...
    return plan != null ? plan.flows : this.flows;
  }
  ...
  _onDragEndFlow () {
    ...
    if (index !== this._draggingFlow.index) {
      const collection = this._findFlowCollection(this._draggingFlow.flow._id);

      const flowToReorder = collection.splice(this._draggingFlow.index, 1)[0];
      collection.splice(index, 0, flowToReorder);
      const min = Math.min(this._draggingFlow.index, index);
      const max = Math.max(this._draggingFlow.index, index);
      collection.forEach((flow, i) => {
        if (i >= min && i <= max) {
          flow._dirty = true;
        }
      });

      dispatchCustomEvent(this, 'update-flows', { flows: this.flows });
      setTimeout(() => {
      ...
      dirty: content._dirty,
          ...
          <div title="${content.name} | Compare with current selection (Shift + click)"
               ...
               class="entry-name link">
                 ${getFlowName(content, true, this.sortable, true)}
                </div>
  ...
  _renderPolicies (filteredData, type, isChild, id) {
    return html`<div class="${classMap({
      content: true,
      expandable: true,
      open: true,
      child: isChild,
    })}" id="${id}">${filteredData.map((content) =>
      html`<div class="${classMap({ entry: true, link: true, policy: true, selected: this._isSelected(content.id) })}" 
                draggable="${true}" 
                ...
                title="Show documentation of ${content.name}">
                  <gv-image src="${content.icon}"></gv-image>
                  <div class="policy-name">${content.name}</div>
                  <gv-icon class="draggable-icon" shape="design:arrows"></gv-icon>
           </div>`)}`;
  }
    ...
    if ((hideIfEmpty && data.length > 0) || !hideIfEmpty) {
      if (groupKey && !isChild) {
        ...
        if (type === 'flows') {
          ...
          list = this._renderPolicies(filteredData, type, isChild, listId);
        }
      }
  ...
  render () {
    ...
    return html`
          ...
          <div class="box">
           ${this.plans != null ? this._renderPart('flows', '', 'shopping:sale#2', this.plans, false, this._onAddFlowToPlan, 'name', false) : ``}
